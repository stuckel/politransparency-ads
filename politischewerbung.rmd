...
#dates
dates_sel<-list(
  "1. Quartal 2023"=as.Date("2023-04-01"),
  "2. Quartal 2023"=as.Date("2023-06-30"))
...
# sliderInput("input_date_target",label="Datum",min=min(target_det$Datum), max=max(target_det$Datum),step=90, value=c(min(target_det$Datum),max(target_det$Datum)),
...
target_gen_data <- reactive({
  ...
  scale_y_continuous(expand = c(0, 0),breaks=seq(0,1,0.1),limits=c(0,1.01),labels = scales::percent)+  facet_grid(.~Akteur_type,scales="free",space="free")+theme(panel.grid.major.y=element_line(color="grey"))+labs(title="",caption="Daten: Meta-Werbebibliothek")
    
    # labs(title="",caption=paste0("Daten: Meta-Werbebibliothek, Zeitraum: ",format(input$input_date_target[1]-89,"%d.%m")," - ",format(input$input_date_target[2],"%d.%m")))
...
target_int_data <- reactive({
  ...
  mutate(Ausgeschlossen=ifelse(is.na(Ausgeschlossen),"Eingeschlossen",as.character(Ausgeschlossen))) %>% 
  mutate(Ausgeschlossen=factor(Ausgeschlossen,levels=c("Eingeschlossen","Ausgeschlossen"))) %>% 
  mutate(Zielgruppe_de=case_when(
    is.na(Zielgruppe_de) & id==2 ~ str_pad("",1),
    is.na(Zielgruppe_de) & id==3 ~ str_pad("",2),
    is.na(Zielgruppe_de) & id==4 ~ str_pad("",3),
    is.na(Zielgruppe_de) & id==5 ~ str_pad("",4),
    is.na(Zielgruppe_de) & id==6 ~ str_pad("",5),
    is.na(Zielgruppe_de) & id==7 ~ str_pad("",6),
    is.na(Zielgruppe_de) & id==8 ~ str_pad("",7),
    is.na(Zielgruppe_de) & id==9 ~ str_pad("",8),
    is.na(Zielgruppe_de) & id==10 ~ str_pad("",9),
    is.na(Zielgruppe_de) & id==11 ~ str_pad("",10),
    is.na(Zielgruppe_de) & id==12 ~ str_pad("",11),
    is.na(Zielgruppe_de) & id==13 ~ str_pad("",12),
    is.na(Zielgruppe_de) & id==14 ~ str_pad("",13),
    is.na(Zielgruppe_de) & id==15 ~ str_pad("",14),
    is.na(Zielgruppe_de) & id==16 ~ str_pad("",15),
    is.na(Zielgruppe_de) & id==17 ~ str_pad("",16),
    is.na(Zielgruppe_de) & id==18 ~ str_pad("",17),
    is.na(Zielgruppe_de) & id==19 ~ str_pad("",18),
    is.na(Zielgruppe_de) & id==20 ~ str_pad("",19),
    TRUE  ~ Zielgruppe_de))
  })
  ...
  theme(axis.ticks = element_blank(),legend.position="bottom",text=element_text(size=13,family="sans"))+
  scale_x_continuous(expand = c(0, 0),breaks=seq(0,1,0.1),limits=c(0,1.01),labels = scales::percent)+labs(title="")+
  facet_wrap(.~Akteur,scales="free",ncol=1)+theme(panel.grid.major.x=element_line(color="grey"))+labs(title="",caption="Daten: Meta-Werbebibliothek.")
...
### Interessen (Top 20) {data-height=1500 .no-mobile}
...
target_int_data <- reactive({
  ...
  ggtitle("") + labs(x="Anteil der Ausgaben",y="",fill="")+scale_fill_viridis_d(direction=-1,begin=0,end=0.9)+
  theme(axis.ticks = element_blank(),legend.position="bottom",text=element_text(size=13,family="sans"))+
  scale_x_continuous(expand = c(0, 0),breaks=seq(0,1,0.1),limits=c(0,1),labels = scales::percent)+labs(title="")+
  facet_wrap(.~Akteur,scales="free",ncol=2)+
      theme(panel.grid.major.x=element_line(color="grey"))+labs(title="",caption="Daten: Meta-Werbebibliothek.")
  ...
  ##make it adaptable to data, but makes comparison between different times more difficult
    # scale_x_continuous(expand = c(0, 0),breaks=seq(0,round(max(target_int_data()$value),1)+0.1,0.1),limits=c(0,round(max(target_int_data()$value),1)+0.1),labels = scales::percent)+labs(title="")+

  #, nur Zielgruppen mit Ausgaben >1 % werden angezeigt
...
target_demo_data <- reactive({
 ...
 top10<-x %>% 
  ...
  theme(axis.ticks = element_blank(),legend.position="bottom",text=element_text(size=13,family="sans"))+
  scale_x_continuous(expand = c(0, 0),breaks=seq(0,1,0.1),limits=c(0,1.01),labels = scales::percent)+labs(title="")+
  facet_wrap(.~Akteur,scales="free",ncol=2)+theme(panel.grid.major.x=element_line(color="grey"))+labs(title="",caption="Daten: Meta-Werbebibliothek.")
...
<br>Fehlermeldung: In einer früheren Version der Webseite waren die Daten zum Targeting im 1. Quartal fehlerhaft. Der Anteil der Ausgaben, den die Parteien für Zielgruppen ausgeben, war zu hoch angegeben, weil gewisse Parteiseiten nicht berücksichtigt wurden. 
