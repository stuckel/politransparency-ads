...
target_gen_data <- reactive({
  ...
  scale_y_continuous(expand = c(0, 0),breaks=seq(0,1,0.1),limits=c(0,1.01),labels = scales::percent)+labs(title="",caption=paste0("Données: Bibliothèque publicitaire Meta, période: ",format(input$input_date_target[1]-89,"%d.%m")," - ",format(input$input_date_target[2],"%d.%m")))+  facet_grid(.~Akteur_type_fr,scales="free",space="free")
...
target_int_data <- reactive({
  ...
  theme(axis.ticks = element_blank(),legend.position="bottom",text=element_text(size=13,family="sans"))+
  scale_x_continuous(expand = c(0, 0),breaks=seq(0,1,0.2),limits=c(0,1.01),labels = scales::percent)+labs(title="",caption=paste0("Données: Bibliothèque publicitaire Meta, période: ",format(input$input_date_target[1]-89,"%d.%m")," - ",format(input$input_date_target[2],"%d.%m")))+
  facet_wrap(.~Akteur_fr,scales="free_y",ncol=1)
...
target_int_data <- reactive({
  ...
  theme(axis.ticks = element_blank(),legend.position="bottom",text=element_text(size=13,family="sans"))+
  scale_x_continuous(expand = c(0, 0),breaks=seq(0,1,0.2),limits=c(0,1.01),labels = scales::percent)+labs(title="",caption=paste0("Données: Bibliothèque publicitaire Meta, période: ",format(input$input_date_target[1]-89,"%d.%m")," - ",format(input$input_date_target[2],"%d.%m")))+
  facet_wrap(.~Akteur_fr,scales="free_y",ncol=2)
...
Les annonces des principaux groupes d'intérêt suisses (Unions des arts et métiers, Unions patronales, Economiesuisse, Unions des paysans, IG Wirtschaft, Unions syndicales) sont prises en compte. Les associations cantonales et nationales sont prises en compte. Toutes les annonces d'un coût supérieur à 100 CHF sont incluses. D'autres groupes d'intérêt avec des dépenses importantes sont ajoutés si nécessaire.
 ...
 <i>Open Data</i> <br>
Le code de ce site web ainsi que les données du rapport publicitaire Meta sont disponibles sur [GitHub](https://github.com/stuckel/politransparency-ads){target="_blank"}.
